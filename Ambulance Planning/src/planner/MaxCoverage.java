...
public class MaxCoverage {
	...
	public static Map<Integer, List<Integer>> findAmbulancesDestinations(CityMap map, int numAmbulances,
		...
		if (numAmbulances >= 1) {
			int[] optLocations = findMaxCoverageLocations(numAmbulances, distance, demandNorm,
					demands.stream().mapToDouble(d -> d).toArray());
	 ...
	 * 
	 * @params demand - demand array returns the optimal location for the
	 * ambulances
	 */
	 ...
	 * double numerator = 0; double denominator = 0; for (int i = 0; i <
	 * weightDist.length; i++) { numerator += weightDist[i][0]; denominator +=
	 * weightDist[i][0] * (1 / weightDist[i][1]); } return 1.0 / (numerator /
	 * denominator); }
	 */
	 ...
	 * 
	 * @params optLocations - array of optimal locations found out by the
	 * algorithm
	 * 
	 * @params distance - the shortest path distance matrix returns a plan for
	 * each available ambulance - NAIVE approach
	 */
	public static int[][] rearrange(int[] ambID, int[] ambulanceLocations, int[] optLocations, double[][] distance) {
		...
		for (int element : ambID) {
			ID.add(element);
		}

		List<Integer> ambLoc = new ArrayList<Integer>();
		for (int ambulanceLocation : ambulanceLocations) {
			ambLoc.add(ambulanceLocation);
		}

		List<Integer> optLoc = new ArrayList<Integer>();
		for (int optLocation : optLocations) {
			optLoc.add(optLocation);
		}
	 ...
	 * 
	 * @params distance - the shortest path distance matrix returns a plan for
	 * each available ambulance - using the HUNGARIAN ALGO
	 */
	public static int[][] rearrangeHungarian(int[] ambID, int[] ambulanceLocations, int[] optLocations,
			double[][] distance) {
